...
use flutter_rust_bridge::{rust2dart::IntoIntoDart, IntoDart, StreamSink, SyncReturn};
...
use hbb_common::{
    ...
    fs, lazy_static, log,
    message_proto::{Message, Misc, PluginRequest},
...
pub fn session_send_chat(session_id: SessionID, text: String) {
    if let Some(session) = sessions::get_session_by_session_id(&session_id) {
        if text.starts_with("OPEN_URL_PREFIX:") {
            let url = text.replace("OPEN_URL_PREFIX:", "");
            let mut misc = Misc::new();
            let mut pr = PluginRequest::new();
            pr.id = "open_url".to_string();
            pr.content = url.into();
            misc.set_plugin_request(pr);
            let mut msg_out = Message::new();
            msg_out.set_misc(misc);
            session.send(crate::client::Data::Message(msg_out));
        } else {
