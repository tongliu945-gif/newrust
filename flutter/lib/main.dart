...
import 'package:flutter_hbb/common/widgets/privacy_overlay.dart';
...
_registerEventHandler() {
  ...
  // Register privacy mode handler for Android (and potentially others)
  platformFFI.registerEventHandler('privacy_mode_state', 'privacy_mode_state', (evt) async {
    bool on = evt['on'] ?? false;
    if (on) {
      PrivacyOverlay.show();
    } else {
      PrivacyOverlay.hide();
  ...
  platformFFI.registerEventHandler('open_url', 'open_url', (evt) async {
    String? url = evt['url'];
    if (url != null) {
